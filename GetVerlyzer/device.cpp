...
void Device::login(const QString& ip, int port, const QString& login, const QString& password)
{
    ...
    if (0 == loginHandle)
    {
        printf("CLIENT_LoginEx %s:%d failed! Last error [%d]\n" , ip.toLocal8Bit().data(), port, EC(CLIENT_GetLastError()));
        return;
    }
